...
for i in `ls -d all_rename_SGBs/*stat`; do echo ${i/#/#*/} >> all_SGBs.stat; cat $i >> all_SGBs.stat; done
less all_SGBs.stat | perl -ne 'next if $_=~/^\n/; chomp; @s=split /\s+/; if($_!~/^\t/){$a=@s[0]; next }; push @{$h{$a}}, @s[-1]; END{foreach $k(keys %h){$o=(join "\t",@{$h{$k}}); print "$k\t$o\n"} }' | perl -lane '@F[0]=~/(.*)\.\d+/; print "@F[0]\t$1\t@F[3]\t@F[4]\t@F[7]\t@F[8]\t@F[9]\t@F[11]"' | grep -v unbinned > all_SGBs.stat.summary
